...
def calculate_all_indicators(df: pd.DataFrame, params: Dict, chunk_size=1000) -> pd.DataFrame:
    ...
    logger.info(f"Validating data quality for {len(df)} rows")
    ...
    if missing_cols:
        ...
        return df  # Return original dataframe if missing critical columns
    ...
    # 3. Fix negative/zero close prices which would cause calculation errors
    if (df['close'] <= 0).any():
    ...
    # === EXISTING CALCULATION LOGIC CONTINUES UNCHANGED ===
